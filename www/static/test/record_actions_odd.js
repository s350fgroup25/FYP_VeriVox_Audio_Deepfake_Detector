let mediaRecorder = null;           // 不再用 MediaRecorder 錄檔，只用來關流（可選）
let audioChunks = [];               // 不再存 webm chunk
let recordedBlob = null;
let recordedFile = null;
let previewUrl = null;

document.addEventListener('DOMContentLoaded', function () {
    // ✅ 先檢查瀏覽器是否支援 getUserMedia
    ...
    let scriptNode = null;
    let pcmData = [];     // 累積 Float32 samples (mono)
    let recording = false;
    ...
    async function startRecording() {
        ...
        try {
            ...
            audioContext = new (window.AudioContext || window.webkitAudioContext)({
                sampleRate: 16000   // 直接用 16kHz，跟模型一致
            });

            mediaSource = audioContext.createMediaStreamSource(mediaStream);

            // ScriptProcessor 已 deprecated，但在大部分瀏覽器仍可用；簡單起見用它
            ...
            scriptNode.connect(audioContext.destination); // 或 audioContext.destination 以避免一些瀏覽器暫停 callback
    ...
    function stopRecording() {
        ...
        // 拼接 Float32 samples → 單一 Float32Array
        let length = 0;
    ...
    submitBtn.onclick = async function() {
        ...
        formData.append('file', recordedFile);  // ✅ WAV
