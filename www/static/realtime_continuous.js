...
async function analyzeChunk(wavBlob, chunkNum, rms) {
    try {
        ...
        if (analyzeJson.success) {
            ...
            addResultLine(chunkNum, isReal ? '✅ REAL' : '❌ FAKE', isReal ? 'green' : 'red', 
              output.prob_real.toFixed(4), output.prob_fake.toFixed(4), rms);
...
// 🔥 修改顯示：Real: 0.0069, Fake: 0.9931 格式
function addResultLine(chunkNum, status, colorClass, realProb, fakeProb, rms) {
    ...
    div.innerHTML = `
        ...
        <span class="prob-col">Real: ${realProb}, Fake: ${fakeProb} | Vol:${(rms*100).toFixed(1)}%</span>
    `;
