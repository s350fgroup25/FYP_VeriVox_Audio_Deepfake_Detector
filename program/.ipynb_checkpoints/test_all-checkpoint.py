...
output_path = os.path.join(output_dir, 'test_output_all.txt')
...
with open(output_path, 'w') as fout, redirect_stdout(fout):
    ...
    # Compute EER
    eer_value, threshold = eer(positive_class_scores, labels)
    print("Equal Error Rate (EER):", eer_value)

    # Attempt to map per-sample file names and durations
    try:
        file_names = []
        durations_ms = []
    ...
    for i in range(min_len):
        fname = file_names[i]
        pred = int(predictions[i].argmax(-1)) if not isinstance(predictions[i], (int, np.integer)) else int(predictions[i])
